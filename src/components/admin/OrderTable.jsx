        ...
        <div className="py-16 text-center">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-gray-500">
              {search ? "Tidak ditemukan pesanan dengan kata kunci tersebut" : "Belum ada pesanan"}
            </p>
            {search && (
              <button
                onClick={() => setSearch("")}
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-4 text-center text-sm font-medium text-gray-500">Qty</th>
              <th className="px-6 py-4 text-center text-sm font-medium text-gray-500">Total</th>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <img
                        src={order.produk?.foto || "https://via.placeholder.com/60"}
                      ...
                      <div>
                        <div className="font-medium text-gray-900">
                          {order.produk?.nama_produk || "Produk tidak ditemukan"}
                        </div>
                        <div className="text-xs text-gray-400 mt-1">
                          ID: {order.id_pesanan}
                        </div>
                        <div className="text-xs text-gray-500 mt-1">
                          {order.catatan ? `"${order.catatan}"` : "Tidak ada catatan"}
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    {order.user ? (
                      <div>
                        ...
                        <div className="text-xs text-gray-400 mt-1">
                          {order.user.no_hp || "Tidak ada telepon"}
                        </div>
                      </div>
                    ) : (
                      <span className="text-gray-400 text-sm">Data customer tidak ditemukan</span>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4 text-center">
                    ...
                    {paymentStatus.isPaid && (
                      <div className={`text-xs mt-1 ${paymentStatus.isFullyPaid ? 'text-green-600' : 'text-orange-600'}`}>
                        {paymentStatus.isFullyPaid ? '✅ Lunas' : '💰 DP dibayar'}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-center">
                      <OrderStatusBadge 
                        status={orderService.mapStatusToFrontend(order.status_pesanan)} 
